...
def extract(grid_path, frame_width, frame_height, grid_rows, grid_cols, split_path):
    grid_images = sorted(os.listdir(grid_path))
    frame_count = 0
    pbar = tqdm(total=len(grid_images), unit='frame')
    ...
    for grid_file in grid_images:
        ...
        for row in range(grid_rows):
            for col in range(grid_cols):
                ...
                output_filename = os.path.join(split_path, f'{frame_count:05d}.png')
